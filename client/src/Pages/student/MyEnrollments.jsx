      ...
      <table className='md:table-auto table-fixed w-full overflow-hidden border mt-10'>
        <thead className='text-gray-900 border-b border-gray-500/20 text-sm text-left max-sm:hidden'>
          <tr>
            ...
            <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>Course Status</th>
          </tr>
        </thead>
        <tbody className='text-gray-700'>
          {enrolledCourses.map((course, index) => (
            <tr key={index} className='border-b border-gray-500/20'>
              ...
              <td className='px-4 py-3 max-sm:hidden'>
               {progressArray[index] && `${progressArray[index].totalLectures} / ${progressArray[index].lectureCompleted}`}<span>Lectures</span> 
              </td>
              <td className='px-4 py-3 max-sm:text-right'>
                <button  className='px-3 sm:px-5 py-1.5 sm:py-2 sm:pu-2 bg-blue-600 max-sm:text-xs text-white'
